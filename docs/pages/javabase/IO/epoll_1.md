...
因为epoll的重要性，不少游戏公司在招聘服务端同学时，可能会问及epoll相关的问题。比如epoll和select的区别是什么？epoll高效率的原因是什么？如果只靠背诵，显然不能算上深刻的理解。  
网上虽然也有不少讲解epoll的文章，但要不是过于浅显，就是陷入源码解析，很少能有通俗易懂的。于是决定编写此文，让缺乏专业背景知识的读者也能够明白epoll的原理。文章核心思想是：要让读者清晰明白epoll为什么性能好。  
...
![0](./epoll_1.assets/38904.png)

工作队列中有A、B和C三个进程

- 等待队列

当进程A执行到创建socket的语句时，操作系统会创建一个由文件系统管理的socket对象（如下图）。这个socket对象包含了**发送缓冲区、接收缓冲区、等待队列**等成员。等待队列是个非常重要的结构，它指向所有需要等待该socket事件的进程。

![0](./epoll_1.assets/38900.png)

- 创建socket

当程序执行到recv时，操作系统会将进程A从工作队列移动到该socket的等待队列中（如下图）。由于工作队列只剩下了进程B和C，依据进程调度，cpu会轮流执行这两个进程的程序，不会执行进程A的程序。所以进程A被阻塞，不会往下执行代码，也不会占用cpu资源。

![0](./epoll_1.assets/38901.png)
...
![0](./epoll_1.assets/38909.png)

内核接收数据全过程

唤醒进程的过程如下图所示。

![0](./epoll_1.assets/38907.png)
...
穿插一个容易忽略的知识点：  
当Java服务端起一个ServerSocket时，每当有一个客户端连接时，都会**创建一个新的Socket对象**。  
这是因为ServerSocket是用来监听客户端连接请求的，一旦有客户端连接请求到来，就会创建一个新的Socket对象与该客户端建立通信连接。  
这个新的Socket对象与客户端之间的通信是独立于其他客户端的，因此需要为每个客户端连接创建一个新的Socket对象。  
所以，如果有多个客户端连接到服务端，就会有多个Socket对象与服务端通信。
这个新的Socket对象的本地端口号是**随机分配**的，通常是一个大于1024且小于65535的整数。  
这个端口号是由操作系统自动分配的，服务端和客户端都可以通过Socket对象的getLocalPort()方法获取本地端口号。   
客户端可以通过Socket对象的getPort()方法获取服务端的端口号。  

select的流程

select的实现思路很直接。假如程序同时监视如下图的socket1、socket2和socket3三个socket，那么在调用select之后，操作系统把进程A分别加入这三个socket的等待队列中。

![0](./epoll_1.assets/38910.png)
...
![0](./epoll_1.assets/38908.png)

socket2接收到了数据，中断程序唤起进程A

所谓唤起进程，就是将进程从所有的等待队列中移除，加入到工作队列里面。如下图所示。

![0](./epoll_1.assets/38911.png)
...
![0](./epoll_1.assets/38912.png)

相比select，epoll拆分了功能

为方便理解后续的内容，我们先复习下epoll的用法。如下的代码中，
...
select低效的另一个原因在于程序不知道哪些socket收到数据，只能一个个遍历。如果内核维护一个“就绪列表”，引用收到数据的socket，就能避免遍历。如下图所示，计算机共有三个socket，收到数据的socket2和socket3被rdlist（就绪列表）所引用。当进程被唤醒后，只要获取rdlist的内容，就能够知道哪些socket收到数据。

![0](./epoll_1.assets/38913.png)
...
创建epoll对象后，可以用epoll_ctl添加或删除所要监听的socket。以添加socket为例，如下图，如果通过epoll_ctl添加socket1、socket2和socket3的监视，内核会将eventpoll添加到这三个socket的等待队列中。

![0](./epoll_1.assets/38923.png)
...
当socket收到数据后，中断程序会给eventpoll的“就绪列表”添加socket引用。如下图展示的是socket2和socket3收到数据后，中断程序让rdlist引用这两个socket。

![0](./epoll_1.assets/38921.png)
...
![0](./epoll_1.assets/38924.png)

epoll_wait阻塞进程

当socket接收到数据，中断程序一方面修改rdlist，另一方面唤醒eventpoll等待队列中的进程，进程A再次进入运行状态（如下图）。也因为rdlist的存在，进程A可以知道哪些socket发生了变化。

![0](./epoll_1.assets/38922.png)
